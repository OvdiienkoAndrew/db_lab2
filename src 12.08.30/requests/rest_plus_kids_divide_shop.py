...
def request(entries,root, name_db):
    ...
    cursor.execute("""
        SELECT 
            ROUND(
                100.0 * 
                SUM(CASE WHEN ТУРИСТ.категорія IN ('Rest', 'Kids') THEN 1 ELSE 0 END) / 
                NULLIF(SUM(CASE WHEN ТУРИСТ.категорія = 'Cargo' THEN 1 ELSE 0 END), 0), 
                2
            ) AS відсоток
        FROM ТУРИСТ
        LEFT JOIN РЕЙС ON РЕЙС.турист_id = ТУРИСТ.id
        WHERE 
            (РЕЙС.дата_час_початку BETWEEN ? AND ?) OR
            (РЕЙС.дата_час_кінця BETWEEN ? AND ?)
    ...
    tree = ttk.Treeview(frame, columns=("Percent"))
    tree["show"] = "headings"
    tree.heading("#1", text="Percent")
...
def rest_plus_kids_divide_shop(root, name_db):
