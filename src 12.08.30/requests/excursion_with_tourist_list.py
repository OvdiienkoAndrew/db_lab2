...
def request(entries,root, name_db):
    ...
    tree = ttk.Treeview(frame, columns=("Excursion name", "Agency name", "Count tourists"))
    tree["show"] = "headings"
    tree.heading("#1", text="Excursion name")
    tree.heading("#2", text="Agency numbers")
...
def excursion_with_tourist_list(root, name_db):
